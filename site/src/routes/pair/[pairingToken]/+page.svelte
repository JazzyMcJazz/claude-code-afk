<script lang="ts">
	...
	let notificationsSupported = $derived(
		browser && 'serviceWorker' in navigator && 'PushManager' in window
	);
		...
		try {
			...
			if (err instanceof Error) {
				...
				if (err.message.includes('denied')) {
					...
					errorMessage =
						'Unable to set up push notifications. This may be due to browser restrictions or network issues. Try using Chrome or Firefox.';
		...
		{:else if status === 'success'}
			<div class="text-green-600 dark:text-green-400">
				...
				<p class="mt-2 text-gray-600 dark:text-gray-300">
					Notifications are now enabled. You can close this page.
				</p>
			</div>
		{:else if status === 'error'}
			<div class="text-red-600 dark:text-red-400">
				...
				<button
					...
					class="mt-4 rounded bg-gray-200 px-4 py-2 font-semibold text-gray-800 transition-colors hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600"
				>
					Try Again
				</button>
			</div>
